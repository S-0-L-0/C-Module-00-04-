...
void testFragTrapSpecialAbility() {
	...
	cheerful.highFivesGuys();
	cheerful.takeDamage(100);
	cheerful.highFivesGuys();
}
...
void testPolymorphicBehaviorAllClasses() {
	...
	clap_ptr->attack("target1");
	scav_ptr->attack("target2");
	frag_ptr->attack("target3");
	delete clap_ptr;
	delete scav_ptr;
	delete frag_ptr;
}
